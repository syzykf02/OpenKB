...
def _log(logger_obj, *args):
    """Emit a progress line through the caller's logger, falling back to the
    module logger when ``logger_obj`` is None.

    Vendored change: progress markers were originally ``print()`` (always
    emitted); several pipeline functions accept ``logger=None`` and some call
    sites (check_toc -> find_toc_pages, meta_processor -> verify_toc) pass no
    logger. Routing through this helper keeps those markers visible without
    crashing on a None logger.
    """
    (logger_obj if logger_obj is not None else logger).info(*args)
...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None):
    ...
    check_cancelled()  # vendored: don't let return_exceptions swallow a cancel
...
def detect_page_index(toc_content, model=None):
    logger.info('start detect_page_index')
    prompt = f"""
...
def toc_index_extractor(toc, content, model=None):
    logger.info('start toc_index_extractor')
...
def find_toc_pages(start_page_index, page_list, opt, logger=None):
    _log(logger, 'start find_toc_pages')
...
def page_list_to_group_text(page_contents, token_lengths, max_tokens=20000, overlap_page=1):    
    ...
    logger.info('divide page_list to groups', len(subsets))
    return subsets
...
def generate_toc_continue(toc_content, part, model=None):
    logger.info('start generate_toc_continue')
    prompt = """
...
def generate_toc_init(part, model=None):
    logger.info('start generate_toc_init')
    prompt = """
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    _log(logger, f'len(group_texts): {len(group_texts)}')

    toc_with_page_number= generate_toc_init(group_texts[0], model)
    for group_text in group_texts[1:]:
        toc_with_page_number_additional = generate_toc_continue(toc_with_page_number, group_text, model)    
        toc_with_page_number.extend(toc_with_page_number_additional)
    _log(logger, f'generate_toc: {toc_with_page_number}')

    toc_with_page_number = convert_physical_index_to_int(toc_with_page_number)
    _log(logger, f'convert_physical_index_to_int: {toc_with_page_number}')
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, logger=None):
    ...
    _log(logger, f'toc_transformer: {toc_content}')
    for page_index in range(start_index, start_index+len(page_list)):
        page_text = f"<physical_index_{page_index}>\n{page_list[page_index-start_index][0]}\n<physical_index_{page_index}>\n\n"
        page_contents.append(page_text)
        token_lengths.append(count_tokens(page_text, model))
    ...
    _log(logger, f'add_page_number_to_toc: {toc_with_page_number}')
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None):
    ...
    _log(logger, f'toc_with_page_number: {toc_with_page_number}')
    ...
    _log(logger, f'toc_with_physical_index: {toc_with_physical_index}')

    toc_with_physical_index = convert_physical_index_to_int(toc_with_physical_index)
    _log(logger, f'toc_with_physical_index: {toc_with_physical_index}')

    matching_pairs = extract_matching_page_pairs(toc_with_page_number, toc_with_physical_index, start_page_index)
    _log(logger, f'matching_pairs: {matching_pairs}')

    offset = calculate_page_offset(matching_pairs)
    _log(logger, f'offset: {offset}')
...
async def fix_incorrect_toc(toc_with_page_number, page_list, incorrect_results, start_index=1, model=None, logger=None):
    _log(logger, f'start fix_incorrect_toc with {len(incorrect_results)} incorrect results')
    ...
    for item, result in zip(incorrect_results, results):
        if isinstance(result, Exception):
            _log(logger, f"Processing item {item} generated an exception: {result}")
            continue
    ...
    _log(logger, f'incorrect_results_and_range_logs: {incorrect_results_and_range_logs}')
    _log(logger, f'invalid_results: {invalid_results}')
...
async def fix_incorrect_toc_with_retries(toc_with_page_number, page_list, incorrect_results, start_index=1, max_attempts=3, model=None, logger=None):
    _log(logger, 'start fix_incorrect_toc')
    fix_attempt = 0
    current_toc = toc_with_page_number
    current_incorrect = incorrect_results

    while current_incorrect:
        _log(logger, f"Fixing {len(current_incorrect)} incorrect results")
        ...
        if fix_attempt >= max_attempts:
            _log(logger, "Maximum fix attempts reached")
            break
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    _log(logger, mode)
    _log(logger, f'start_index: {start_index}')
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        _log(logger, 'large node:', node['title'], 'start_index:', node['start_index'], 'end_index:', node['end_index'], 'token_num:', token_num)
...
def validate_and_truncate_physical_indices(toc_with_page_number, page_list_length, start_index=1, logger=None):
    ...
    _log(logger, f"Document validation: {page_list_length} pages, max allowed index: {max_allowed_page}")
    if truncated_items:
        _log(logger, f"Truncated {len(truncated_items)} TOC items that exceeded document length")
